...
export function HighlightEditor(props) {
    ...
    return throttledEdit || edit ? (<React.Fragment>
      <Textarea value={state.code.value} name="text" placeholder={i18n.code.placeholder} onChange={(e) => {
            state.code.set(e.target.value);
        }}>
        {state.code.value}
      </Textarea>
      <EditorInlineSettings>
        <EditorInput list="available-languages" label={i18n.language.label} value={state.language.value} onChange={(e) => {
            state.language.set(e.target.value);
        }} placeholder={i18n.language.placeholder}/>
        <datalist id="available-languages">
          {['c', 'javascript', 'jsx', 'markup', 'java', 'python'].map((language) => {
            return <option key={language} value={language}/>;
        })}
        </datalist>
        <CheckboxContainer>
          <EditorCheckbox label={i18n.showLineNumbers.label} onChange={() => {
            state.showLineNumbers.set(!state.showLineNumbers.value);
        }} checked={state.showLineNumbers.value}/>
        </CheckboxContainer>
      </EditorInlineSettings>
    </React.Fragment>) : (<Renderer config={config} language={state.language.value} showLineNumbers={state.showLineNumbers.value} code={state.code.value}/>);
}
