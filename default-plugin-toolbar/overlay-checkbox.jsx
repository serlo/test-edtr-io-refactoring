...
const OverlayCheckboxToggleContainer = styled.div(({ value }) => {
    return {
        cursor: 'pointer',
...
const OverlayCheckboxToggle = styled.div(({ value }) => {
    return {
        opacity: value ? 1 : 0,
        content: '',
        position: 'absolute',
...
export function createOverlayCheckbox(_config) {
    return function OverlayCheckbox({ checked, onChange, label, }) {
        return (<OverlayCheckboxLabel>
        <OverlayCheckboxLabelInner>{label}</OverlayCheckboxLabelInner>
        <OverlayCheckboxInner>
          <OverlayCheckboxToggleContainer onClick={() => {
                if (onChange) {
                    onChange(!checked);
                }
            }} value={checked}>
