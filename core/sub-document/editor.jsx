import { change, focus, focusNext, focusPrevious, getDocument, mayRemoveChild, getParent, getPlugin, insertChildAfter, isEmpty, isFocused, redo, removeChild, undo, } from '@edtr-io/store';
...
export function SubDocumentEditor({ id, pluginProps }) {
    ...
    const plugin = useScopedSelector((state) => document && getPlugin(document.plugin)(state));
    const store = useScopedStore();
    const container = React.useRef(null);
    const settingsRef = React.useRef(window.document.createElement('div'));
    const toolbarRef = React.useRef(window.document.createElement('div'));
    ...
    const renderIntoSettings = React.useCallback((children) => {
        return (<RenderIntoSettings setHasSettings={setHasSettings} settingsRef={settingsRef}>
          {children}
        </RenderIntoSettings>);
    }, [settingsRef]);
    const renderIntoToolbar = React.useCallback((children) => {
        return (<RenderIntoToolbar setHasToolbar={setHasToolbar} toolbarRef={toolbarRef}>
          {children}
        </RenderIntoToolbar>);
    }, [toolbarRef]);
    ...
    return React.useMemo(() => {
        ...
        const defaultConfig = typeof plugin.config === 'function' ? plugin.config(theme) : plugin.config;
        const overrideConfig = (pluginProps && pluginProps.config) || {};
        const config = R.mergeDeepRight(defaultConfig, overrideConfig);
        const onChange = (initial, additional = {}) => {
        ...
        return (<HotKeys keyMap={hotKeysKeyMap} handlers={hotKeysHandlers} allowChanges>
        <StyledDocument onMouseDown={handleFocus} ref={container} data-document tabIndex={-1}>
          <DocumentEditor hasSettings={hasSettings} hasToolbar={hasToolbar} focused={focused} renderSettings={pluginProps && pluginProps.renderSettings} renderToolbar={pluginProps && pluginProps.renderToolbar} settingsRef={settingsRef} toolbarRef={toolbarRef} PluginToolbar={PluginToolbar}>
            <plugin.Component renderIntoSettings={renderIntoSettings} renderIntoToolbar={renderIntoToolbar} id={id} editable focused={focused} config={config} state={state} autofocusRef={autofocusRef}/>
          </DocumentEditor>
        </StyledDocument>
      </HotKeys>);
    }, [
...
function RenderIntoSettings({ children, setHasSettings, settingsRef, }) {
    React.useEffect(() => {
        setHasSettings(true);
    });
...
function RenderIntoToolbar({ children, setHasToolbar, toolbarRef, }) {
    React.useEffect(() => {
        setHasToolbar(true);
    });
