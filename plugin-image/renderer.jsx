...
export class ImageRenderer extends React.Component {
    render() {
        ...
        const image = (<ImgWrapper maxWidth={state.maxWidth.defined ? state.maxWidth.value : 0}>
        {!isTempFile(state.src.value) ? (<Img src={state.src.value} alt={alt}/>) : state.src.value.loaded ? (<Uploading>
            <PendingOverlay>
              <Pending />
            </PendingOverlay>
        ...
        return (<div>
        {state.link.defined && state.link.href.value && !disableMouseEvents ? (<a href={state.link.href.value} {...(state.link.openInNewTab.value
                ? {
                    target: '_blank',
