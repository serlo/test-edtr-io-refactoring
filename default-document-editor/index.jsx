import { edtrClose, EdtrIcon, faCog, Icon, merge, styled, } from '@edtr-io/ui';
import * as React from 'react';
const ToolbarContent = styled.div(({ isFocused, isHovered }) => ({
    backgroundColor: '#fff',
...
const ToolbarContainer = styled.div(({ isFocused, isHovered }) => ({
    position: 'absolute',
    top: 0,
    left: 0,
...
const Container = styled.div(({ noHeight, isFocused, isHovered }) => ({
    ...(!noHeight
        ? {
            minHeight: '10px',
            marginBottom: '25px',
 ...
 * @beta
 */
export function createDefaultDocumentEditor(config = {}) {
    return function DocumentEditor({ focused, children, renderSettings, renderToolbar, settingsRef, toolbarRef, hasSettings, hasToolbar, PluginToolbar, }) {
        ...
        const renderSettingsContent = React.useMemo(() => {
            return shouldShowSettings
                ? (children, { close }) => (<React.Fragment>
              <Header>
                <H4>{modalTitle}</H4>
                <BorderlessOverlayButton as={OverlayButton} onClick={() => {
                        close();
        ...
        {showSettings() ? (<PluginToolbarOverlayButton label={i18n.settings.buttonLabel} icon={<Icon icon={faCog} size="lg"/>} renderContent={renderSettingsContent} contentRef={settingsRef}/>) : null}
        <div ref={(ref) => {
        ...
        return (<Container className={isFocused || isHovered ? 'default-document-editor-container' : ''} isFocused={isFocused} isHovered={isHovered} onMouseEnter={() => setHasHover(true)} onMouseLeave={() => setHasHover(false)}>
        {children}
