...
const StyledPlugin = styled.div(({ config }) => {
    const { theme } = config;
    return {
        display: 'flex',
...
export const Plugin = ({ config, plugin, pluginName, onClick, }) => {
    return (<StyledPlugin config={config} onClick={onClick}>
      <IconWrapper>
        {plugin.icon ? (<plugin.icon />) : (<DefaultIcon icon={edtrDefaultPlugin}/>)}
      </IconWrapper>
