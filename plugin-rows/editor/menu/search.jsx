...
const ClearSearchContainer = styled.div(({ config, visible }) => {
    const { theme } = config;
    return {
        height: '55%',
        position: 'absolute',
...
const SearchIcon = styled(EdtrIcon)(({ config }) => {
    const { theme } = config;
    return {
        height: '55%',
        position: 'absolute',
...
export const Search = ({ search, setSearch, config, }) => {
    return (<StyledSearch>
      <InputWrapper>
        <StyledInput config={config} placeholder={config.i18n.menu.searchPlaceholder} value={search} onChange={(e) => setSearch(e.target.value)}/>
        <ClearSearchContainer config={config} visible={search.length > 0} onClick={() => setSearch('')}>
