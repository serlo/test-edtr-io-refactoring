...
const StyledResizable = styled(ReactResizeable)((props) => {
    return {
...
const Floating = styled.div((props) => {
    return {
...
export function Resizable(props) {
    ...
    const [widthInPercent, setWidthInPercent] = React.useState((props.widthInSteps * 100) / props.steps);
    ...
    function onResizeEnd(event, { size }) {
        const newSize = widthToSize(size.width);
        const { onResizeEnd = () => { } } = props;
        onResizeEnd(newSize);
    }
    return (<Floating className={props.className} responsiveBreakpoint={props.responsiveBreakpoint} floating={props.floating} widthInPercent={widthInPercent}>
      {props.enabled ? (<StyledResizable onResize={onResize} onResizeStop={onResizeEnd} draggableOpts={{
                axis: 'none',
                offsetParent: document.body,
            }} width={width} height={0} floating={props.floating}>
          {/* this div needs to be kept or resize will be broken */}
          <div>{props.children}</div>
        </StyledResizable>) : (<div>{props.children}</div>)}
    </Floating>);
