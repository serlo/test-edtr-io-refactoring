...
const OverlayTriangle = styled.div(({ positionAbove = false }) => {
...
export function HoverOverlay(props) {
    ...
    const [positionAbove, setPositionAbove] = React.useState(props.position === 'above');
    ...
    React.useEffect(() => {
        if (!overlay.current || !triangle.current)
            return;
        let rect;
        ...
        menu.style.left = `${Math.max(Math.min(Math.max(rect.left - parentRect.left - menu.offsetWidth / 2 + rect.width / 2, 0), parentRect.width - menu.offsetWidth - 5), 0)}px`;
    ...
    return (<InlineOverlayWrapper ref={overlay}>
