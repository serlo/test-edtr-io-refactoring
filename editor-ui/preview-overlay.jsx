...
const NoClickArea = styled.div((props) => {
    return {
...
export function PreviewOverlay(props) {
    ...
    const setActive = React.useCallback((active) => {
    ...
    return (<NoClickArea active={active}>
      <Overlay blur={props.focused} active={active}>
        {props.focused ? (<ButtonWrapper>
      ...
      {!props.editable ? (<ScopeContext.Provider value={{
                scope,
                editable: false,
            }}>
          {props.children}
        </ScopeContext.Provider>) : (props.children)}
      {active ? (<ButtonWrapper>
          <ActivateButton onClick={() => {
                setActive(false);
            }}>
            Editieren
