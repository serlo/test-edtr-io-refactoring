...
const MathEditorTextArea = (props) => {
    const [latex, setLatex] = React.useState(props.defaultValue);
    const { onChange } = props;
    const parentOnChange = React.useCallback((e) => {
    ...
    return (<EditorTextarea style={mathEditorTextareaStyle} onChange={parentOnChange} onMoveOutRight={props.onMoveOutRight} onMoveOutLeft={props.onMoveOutLeft} value={latex} ref={textareaRef}/>);
};
 ...
 * @public
 */
export function MathEditor(props) {
    ...
    return (<React.Fragment>
      <Modal ariaHideApp={false} isOpen={helpOpen} onRequestClose={() => {
            setHelpOpen(false);
        }} style={{
            overlay: {
                zIndex: 9999,
    ...
    function renderChildren() {
        if (readOnly) {
            return state ? (<MathRenderer {...props}/>) : (<span style={{ backgroundColor: 'lightgrey' }} {...props.additionalContainerProps}>
          {config.i18n.placeholder}
        </span>);
        }
        return (<React.Fragment>
        {useVisualEditor ? (<EditorWrapper onClick={(e) => {
                    e.stopPropagation();
                }} inline={props.inline} ref={anchorRef} {...props.additionalContainerProps}>
            <VisualEditor {...props} onError={() => {
                    setHasError(true);
                }}/>
          </EditorWrapper>) : (<MathRenderer {...props} ref={anchorRef}/>)}
        {helpOpen ? null : (<HoverOverlay position="above" anchor={anchorRef}>
            <div onClick={(e) => {
                    e.stopPropagation();
                }}>
              <Dropdown config={config} value={useVisualEditor ? 'visual' : 'latex'} onChange={(e) => {
                    if (hasError)
                        setHasError(false);
              ...
              {!disableBlock && (<InlineCheckbox label={config.i18n.displayBlockLabel} checked={!props.inline} onChange={(checked) => {
                        if (typeof props.onInlineChange === 'function') {
                            props.onInlineChange(!checked);
                        }
                    }}/>)}
              {useVisualEditor && (<Button config={config} onMouseDown={() => {
                        setHelpOpen(true);
                    }}>
              ...
              <br />
              {!useVisualEditor && (<MathEditorTextArea {...props} defaultValue={state}/>)}
            </div>
          </HoverOverlay>)}
      </React.Fragment>);
    }
