...
const CheckboxToggleContainer = styled.div(({ value }) => {
    return {
        cursor: 'pointer',
...
const CheckboxToggle = styled.div(({ value }) => {
    return {
        opacity: value ? 1 : 0,
        content: '',
        position: 'absolute',
...
export function InlineCheckbox({ checked, onChange, label }) {
    return (<CheckboxInlineLabel>
      <CheckboxInlineLabelInner>{label}</CheckboxInlineLabelInner>
      <CheckboxToggleContainer onMouseDown={(e) => {
            // avoid loosing focus
            e.stopPropagation();
        }} onClick={() => {
            if (onChange) {
