...
function alternativeSaneKeyboard(el_, handler) {
    const el = el_[0];
    el.value = ' ';
...
export function VisualEditor(props) {
    ...
    return (<MQ.EditableMathField latex={props.state} onChange={(ref) => {
            props.onChange(ref.latex());
        }} 
    // @ts-expect-error https://github.com/serlo/serlo-editor-issues-and-documentation/issues/67
    config={mathQuillConfig} mathquillDidMount={onMount}/>);
    function onMount(ref) {
