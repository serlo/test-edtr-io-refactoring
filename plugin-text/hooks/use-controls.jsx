import { edtrBold, edtrClose, edtrFormula, EdtrIcon, edtrItalic, edtrLink, edtrListBullets, edtrListNumbered, edtrText, faCode, Icon, } from '../../ui';
...
import { getColorIndex, isAnyColorActive, isColorActive, resetColor, toggleColor, } from '../utils/color';
import { isLinkActive, toggleLink } from '../utils/link';
import { isOrderedListActive, isUnorderedListActive, toggleOrderedList, toggleUnorderedList, } from '../utils/list';
import { isMathActive, toggleMath } from '../utils/math';
import { isAnyHeadingActive, isBoldActive, isCodeActive, isHeadingActive, isItalicActive, toggleBoldMark, toggleCode, toggleHeading, toggleItalicMark, } from '../utils/rich-text';
const textPluginsMapper = {
    [TextEditorControl.math]: withMath,
    [TextEditorControl.links]: withLinks,
    [TextEditorControl.lists]: withLists,
};
const isRegisteredTextPlugin = (control) => {
    return control in textPluginsMapper;
};
const toggleLinkAndFlag = (setIsLinkNewlyCreated) => (editor) => {
    toggleLink(editor);
    setIsLinkNewlyCreated(true);
};
const registeredHotkeys = (setIsLinkNewlyCreated) => [
    {
        hotkey: 'mod+b',
...
export const useControls = (config, setIsLinkNewlyCreated) => {
    const { controls } = config;
    const createTextEditor = useCallback((baseEditor) => controls.reduce((currentEditor, currentControl) => {
    ...
    const toolbarControls = useMemo(() => createToolbarControls(config, setIsLinkNewlyCreated), [config, setIsLinkNewlyCreated]);
    const handleHotkeys = useCallback((event, editor) => {
        // Go through the registered hotkeys
        for (const { hotkey, control, handler } of registeredHotkeys(setIsLinkNewlyCreated)) {
...
function createToolbarControls({ i18n, theme, controls }, setIsLinkNewlyCreated) {
    const allControls = [
        // Bold
        {
        ...
        // Colors
        {
            ...
            isActive: () => false,
            renderIcon: (editor) => (<HoveringToolbarColorTextIcon index={getColorIndex(editor)} colorsTheme={theme.controls.colors}/>),
            renderCloseMenuIcon: () => <EdtrIcon icon={edtrClose}/>,
            children: [
                {
                    name: TextEditorControl.colors,
                    ...
                    onClick: resetColor,
                    renderIcon: () => (<HoveringToolbarColorIcon color={theme.controls.colors.defaultColor}/>),
                },
    ...
    return allControls.filter((control) => controls.includes(TextEditorControl[control.name]));
}
