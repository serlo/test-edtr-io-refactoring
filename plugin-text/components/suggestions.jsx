...
const Suggestion = styled.div(({ isActive, theme }) => ({
    height: '32px',
    padding: '4px 8px',
    cursor: 'pointer',
...
const StyledText = styled.span(({ isHighlighted, theme }) => ({
    color: isHighlighted
...
export const Suggestions = (props) => {
    ...
    return (<Container>
      {options.map(({ name, title }, index) => {
            ...
            return (<Suggestion key={index} isActive={index === selected} onMouseDown={() => onMouseDown(name)} theme={theme}>
            {fragments.map((fragment, fragmentIndex) => (<StyledText key={fragmentIndex} isHighlighted={fragment.isHighlighted} theme={theme}>
                {fragment.text}
              </StyledText>))}
          </Suggestion>);
        })}
